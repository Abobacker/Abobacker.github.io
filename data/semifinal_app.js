...
function markCurrent(){if(!deck.length||!deck[i])return;let id=deck[i].markId||deck[i].id;if(marks.has(id))marks.delete(id);else marks.add(id);saveMarks();updateMarkedCount();markUI();pace()}
...
function options(){scope.innerHTML=`<option value="all">All minimum questions — 220</option><option value="highlighted">★ Highlighted — ${markedPool().length}</option><option value="general">General microbiology — G1–G66</option><option value="systematic">Systemic bacteriology — S1–S154</option>${C.map(c=>`<option value="c${c.n}">${String(c.n).padStart(2,'0')} · ${c.title} — ${c.start}–${c.end}</option>`).join('')}`}
...
function pace(){let n=targetCount(),s=Math.max(.2,+secq.value||6),h=s/2,e=examPreset(),ord=$('order').value==='random'?'random order':'source order';$('pace').innerHTML=`<span class="tag"><b>${n}</b> prompts</span><span class="tag"><b>${s.toFixed(1)} s</b> each</span><span class="tag">${h.toFixed(1)} s recall + ${h.toFixed(1)} s reveal</span><span class="tag">≈ <b>${(s*n/60).toFixed(1)} min</b></span><span class="tag">${e?`<b>${e[0]} General + ${e[1]} Systemic</b> · fresh draw`:ord}</span><span class="tag">★ <b>${markedPool().length}</b> saved</span>`}
...
function finish(){run=0;paused=0;cancelAnimationFrame(raf);$('run').style.display='none';end.classList.add('on');let e=examPreset(),ord=$('order').value==='random'?'random order':'source order';endsummary.textContent=e?`${e[0]+e[1]}-question oral set complete: ${e[0]} General + ${e[1]} Systemic. Run again for a fresh draw.`:`${deck.length} prompts swept in ${ord}. ${markedPool().length} highlighted for later.`}
...
options();initTheme();modeChange();if(all.length!==220)question.textContent=`Data load error: expected 220 prompts, found ${all.length}.`;
