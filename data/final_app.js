...
const PRESETS={gs44:[4,4,0,0,0],gs48:[4,8,0,0,0],gs416:[4,16,0,0,0],gs816:[8,16,0,0,0],gs08:[0,8,0,0,0],gs016:[0,16,0,0,0]};
...
function pace(){
 ...
 $('pace').innerHTML=`<span class="tag"><b>${n}</b> prompts</span><span class="tag"><b>${s.toFixed(1)} s</b> each</span><span class="tag">${h.toFixed(1)} s recall + ${h.toFixed(1)} s reveal</span><span class="tag">≈ <b>${(s*n/60).toFixed(1)} min</b></span><span class="tag">${mode}</span><span class="tag">★ <b>${markedPool().length}</b> saved</span>`
}
...
function finish(){run=0;paused=0;cancelAnimationFrame(raf);$('run').style.display='none';end.classList.add('on');let ord=$('order').value==='random'?'random order':'source order';if($('runmode').value==='mix'){let c=mixCounts();endsummary.textContent=`${deck.length}-question random final set complete: G ${c[0]} · S ${c[1]} · M ${c[2]} · P ${c[3]} · V ${c[4]}. Run again for a fresh draw.`}else endsummary.textContent=`${deck.length} prompts swept in ${ord}. ${markedPool().length} highlighted for later.`}
